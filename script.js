...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // FIXED: Smooth scroll HANYA untuk internal anchor links, bukan untuk external links
    document.querySelectorAll('a[href^="#"]:not(.external-link)').forEach(anchor => {
        anchor.addEventListener('click', function(e) {
            const targetId = this.getAttribute('href');
            
            // Pastikan ini benar-benar anchor link internal
            if (targetId === '#' || targetId.startsWith('http') || 
                targetId.startsWith('www') || targetId.startsWith('mailto') || 
                targetId.startsWith('tel')) {
                return; // Biarkan link eksternal bekerja normal
            }
    ...
    // Debug untuk link sosial media
    console.log('Debug: External links found:');
    document.querySelectorAll('.external-link').forEach(link => {
        console.log(`- ${link.href} (${link.textContent || link.innerHTML})`);
        link.addEventListener('click', function(e) {
            console.log(`External link clicked: ${this.href}`);
            // Tidak ada preventDefault di sini, biarkan browser menangani
        });
    });
    ...
    // Log untuk debugging
    console.log('Script loaded successfully. Instagram link should work now.');
});